...
const ORBS: Array<[number, number, number, string, number, string, number, number]> = [
  [12, 6, 200, "#006be4", 52, "animate-float-1", 0.0, 0.3],
  [46, 2, 160, "#6173ff", 46, "animate-float-3", 1.2, 0.24],
  [80, 8, 210, "#22c55e", 54, "animate-float-2", 0.6, 0.2],
  [3, 40, 190, "#003168", 50, "animate-bob", 1.6, 0.32],
  [95, 34, 180, "#b5d6ff", 48, "animate-float-4", 0.4, 0.22],
  [6, 82, 170, "#f59e0b", 46, "animate-float-2", 2.0, 0.2],
  [92, 78, 200, "#6173ff", 52, "animate-float-1", 1.1, 0.24],
  [70, 90, 150, "#006be4", 44, "animate-bob", 0.8, 0.22],
];
      ...
      {COINS.map(([left, top, depth, symbol, anim, delay, size], i) => (
        ...
        >
          ...
          <span className="relative block transition-transform duration-300 ease-out group-hover:z-40 group-hover:scale-[1.7] [perspective:600px]">
            ...
            <span className="pointer-events-none absolute left-1/2 top-full mt-2 -translate-x-1/2 whitespace-nowrap rounded-md bg-[#0E1424] px-2 py-0.5 font-mono text-[10px] text-white opacity-0 ring-1 ring-white/10 transition-opacity duration-200 group-hover:opacity-100">
              {symbol}
            </span>
          </span>
        </span>
